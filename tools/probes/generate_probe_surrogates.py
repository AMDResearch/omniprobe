...
def helper_arg_signature(contract: str, captures_type: str) -> tuple[list[str], list[str], list[str]]:
    if contract == "kernel_lifecycle_v1":
        return (
            ...
            [
                "runtime", "captures", "timestamp", "workgroup_x",
                "workgroup_y", "workgroup_z", "thread_x", "thread_y",
                "thread_z", "block_dim_x", "block_dim_y", "block_dim_z",
                "lane_id", "wave_id", "wavefront_size", "hw_id",
                "exec_mask",
            ],
            [
                "kernel_lifecycle_event event{};",
                "event.timestamp = timestamp;",
                "event.workgroup_x = workgroup_x;",
                "event.workgroup_y = workgroup_y;",
                "event.workgroup_z = workgroup_z;",
                "event.thread_x = thread_x;",
                "event.thread_y = thread_y;",
                "event.thread_z = thread_z;",
                "event.block_dim_x = block_dim_x;",
                "event.block_dim_y = block_dim_y;",
                "event.block_dim_z = block_dim_z;",
                "event.lane_id = lane_id;",
                "event.wave_id = wave_id;",
                "event.wavefront_size = wavefront_size;",
                "event.hw_id = hw_id;",

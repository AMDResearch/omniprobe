...
- **Bank Conflict**: Multiple lanes accessing different addresses in same bank → serialization
- **Cache Line**: Unit of memory transfer (typically 64 bytes)
- **Coalesced Access**: Memory access pattern where lanes access consecutive addresses → optimal cache usage
...
## LLVM/Compilation
- **IR (Intermediate Representation)**: LLVM's platform-independent code format

...
- Documentation rewrite for v1.0 complete (`rf_documentation-v1`, done).
  ...
  usage overview table linking to `docs/usage.md` sections. Removed
  history/alpha disclaimers. Existing docs had minor heading capitalization fixes.
  `hip-instrumentation.md` covers compile-time plugin usage (`-fpass-plugin`),
  scoped instrumentation via env vars, CMake integration, `.hsaco` compilation.
  `usage.md` analyzer table shows plugin-to-analyzer mapping.

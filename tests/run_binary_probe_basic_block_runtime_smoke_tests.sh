#!/bin/bash
################################################################################
# Binary-only basic-block probe runtime smoke tests
#
...
#   4. observe a visible side effect from a helper that requires no builtins
################################################################################
...
WORK_DIR="$OUTPUT_DIR/binary_probe_basic_block_runtime_smoke"
CACHE_DIR="$OUTPUT_DIR/binary_probe_basic_block_runtime_smoke_cache"
rm -rf "$WORK_DIR" "$CACHE_DIR"
mkdir -p "$WORK_DIR" "$CACHE_DIR"

SPEC_FILE="$WORK_DIR/module_load_binary_basic_block.yaml"
HELPER_FILE="$WORK_DIR/module_load_binary_basic_block_helper.hip"
BUNDLE_DIR="$WORK_DIR/bundle"
BUNDLE_JSON="$BUNDLE_DIR/generated_probe_bundle.json"
MANIFEST_JSON="$WORK_DIR/module_load_plain.manifest.json"
PLAN_JSON="$WORK_DIR/module_load_binary_basic_block.plan.json"
THUNK_SOURCE="$WORK_DIR/module_load_binary_basic_block.thunks.hip"
THUNK_MANIFEST="$WORK_DIR/module_load_binary_basic_block.thunks.json"
CARRIER_HSACO="$WORK_DIR/module_load_binary_basic_block.carrier.hsaco"
CARRIER_REPORT="$WORK_DIR/module_load_binary_basic_block.carrier.report.json"
CARRIER_MANIFEST="$WORK_DIR/module_load_binary_basic_block.carrier.manifest.json"
RUNTIME_LOG="$WORK_DIR/module_load_binary_basic_block.runtime.log"
...
python3 "${REPO_ROOT}/tools/codeobj/disasm_to_ir.py" \
    ...
    --output "$WORK_DIR/module_load_plain.ir.json" >/dev/null
python3 "$PLAN_PROBE" \
    "$MANIFEST_JSON" \
    --ir "$WORK_DIR/module_load_plain.ir.json" \
...
echo ""
echo "================================================================================"
echo "Binary Probe Basic-Block Runtime Smoke Tests"
echo "================================================================================"
...
TEST_NAME="binary_probe_basic_block_runtime_build"
...
PY
then
    echo -e "  ${GREEN}✓ PASS${NC} - Basic-block binary regeneration produced a hidden-ABI carrier with grown private segment"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Basic-block binary carrier regeneration metadata was not correct"
...
TESTS_RUN=$((TESTS_RUN + 1))
TEST_NAME="binary_probe_basic_block_runtime_exec"
echo -e "\n${YELLOW}[TEST $TESTS_RUN]${NC} $TEST_NAME"
...
if [ "$run_status" -ne 124 ] && \
   ...
   ! grep -q "Memory access fault by GPU" "$RUNTIME_LOG"; then
    echo -e "  ${GREEN}✓ PASS${NC} - Basic-block carrier executed injected mid-kernel helper code and changed kernel output as expected"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Basic-block carrier runtime validation failed"

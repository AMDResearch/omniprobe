#!/bin/bash
################################################################################
# Binary-only lifecycle probe runtime smoke tests
#
# Verifies the current donor-free binary-only path can:
#   1. plan lifecycle instrumentation for an existing hsaco with no source build
#   2. generate kernel-specific thunk glue
#   3. regenerate a hidden-ABI carrier hsaco with linked probe support code
#   4. let Omniprobe select that carrier at module-load time
#   5. execute injected helper code through the runtime-selected hidden-ABI
#      carrier and observe a kernel-visible side effect
################################################################################
...
WORK_DIR="$OUTPUT_DIR/binary_probe_runtime_smoke"
CACHE_DIR="$OUTPUT_DIR/binary_probe_runtime_smoke_cache"
...
PLAN_JSON="$WORK_DIR/module_load_binary_lifecycle.plan.json"
THUNK_SOURCE="$WORK_DIR/module_load_binary_lifecycle.thunks.hip"
THUNK_MANIFEST="$WORK_DIR/module_load_binary_lifecycle.thunks.json"
CARRIER_HSACO="$WORK_DIR/module_load_binary_lifecycle.carrier.hsaco"
CARRIER_REPORT="$WORK_DIR/module_load_binary_lifecycle.carrier.report.json"
CARRIER_MANIFEST="$WORK_DIR/module_load_binary_lifecycle.carrier.manifest.json"
RUNTIME_LOG="$WORK_DIR/module_load_binary_lifecycle.runtime.log"
...
echo ""
echo "================================================================================"
echo "Binary Probe Runtime Smoke Tests"
echo "================================================================================"
...
TEST_NAME="binary_probe_runtime_carrier_build"
...
PY
then
    echo -e "  ${GREEN}✓ PASS${NC} - Binary-only regeneration produced a hidden-ABI carrier with linked probe support"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Binary-only carrier regeneration metadata was not correct"
...
TESTS_RUN=$((TESTS_RUN + 1))
TEST_NAME="binary_probe_runtime_carrier_exec"
echo -e "\n${YELLOW}[TEST $TESTS_RUN]${NC} $TEST_NAME"
...
if [ "$run_status" -ne 124 ] && \
   ...
   grep -q "MISMATCH at index 0: expected 0, got 1234" "$RUNTIME_LOG" && \
   grep -q "module_load_test: FAIL" "$RUNTIME_LOG" && \
   ! grep -q "Memory access fault by GPU" "$RUNTIME_LOG"; then
    echo -e "  ${GREEN}✓ PASS${NC} - Binary-only carrier executed injected helper code and changed kernel output as expected"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Binary-only carrier runtime validation failed"

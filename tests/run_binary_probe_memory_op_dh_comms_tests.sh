#!/bin/bash
################################################################################
# Binary-only memory-op dh_comms runtime smoke tests
#
# Verifies the donor-free binary-only path can:
#   1. plan memory_op instrumentation for an existing hsaco with no source build
#   2. regenerate a hidden-ABI carrier hsaco with linked probe support code
#   3. execute a mid-kernel helper call from a memory-op insertion point
#   4. emit host-visible dh_comms traffic only when the decoded memory address
#      falls inside the target kernel argument buffer
################################################################################
...
WORK_DIR="$OUTPUT_DIR/binary_probe_memory_op_dh_comms"
CACHE_DIR="$OUTPUT_DIR/binary_probe_memory_op_dh_comms_cache"
rm -rf "$WORK_DIR" "$CACHE_DIR"
mkdir -p "$WORK_DIR" "$CACHE_DIR"

SPEC_FILE="$WORK_DIR/module_load_binary_memory_op_dh_comms.yaml"
HELPER_FILE="$WORK_DIR/module_load_binary_memory_op_dh_comms_helper.hip"
...
PLAN_JSON="$WORK_DIR/module_load_binary_memory_op_dh_comms.plan.json"
THUNK_SOURCE="$WORK_DIR/module_load_binary_memory_op_dh_comms.thunks.hip"
THUNK_MANIFEST="$WORK_DIR/module_load_binary_memory_op_dh_comms.thunks.json"
CARRIER_HSACO="$WORK_DIR/module_load_binary_memory_op_dh_comms.carrier.hsaco"
CARRIER_REPORT="$WORK_DIR/module_load_binary_memory_op_dh_comms.carrier.report.json"
CARRIER_MANIFEST="$WORK_DIR/module_load_binary_memory_op_dh_comms.carrier.manifest.json"
RUNTIME_LOG="$WORK_DIR/module_load_binary_memory_op_dh_comms.runtime.log"

cat > "$SPEC_FILE" <<'SPEC'
version: 1

helpers:
  source: module_load_binary_memory_op_dh_comms_helper.hip
...
probes:
  - id: module_load_binary_memory_op_dh_comms
    target:
      kernels: ["mlk"]
      match:
        kind: isa_mnemonic
    ...
    inject:
      when: memory_op
      helper: module_load_binary_memory_op_dh_comms_probe
...
extern "C" __device__ void module_load_binary_memory_op_dh_comms_probe(
    const helper_args<omniprobe_user::module_load_binary_memory_op_dh_comms_captures,
                      memory_op_event> &args) {
  if (args.event->access != memory_access_kind::write) {
    return;
  }
  if (args.event->address_space != address_space_kind::flat) {
    return;
  }
  ...
  auto *data = reinterpret_cast<uint32_t *>(static_cast<uintptr_t>(args.captures->data));
  const size_t size = static_cast<size_t>(args.captures->size);
  const uintptr_t base = reinterpret_cast<uintptr_t>(data);
  const uintptr_t limit = base + size * sizeof(uint32_t);
  const uintptr_t address = static_cast<uintptr_t>(args.event->address);
  if (address < base || address >= limit) {
    return;
  }
  if (((address - base) % sizeof(uint32_t)) != 0) {
    return;
  }

  dh_comms::v_submit_address(
      args.runtime->dh, reinterpret_cast<void *>(address), 0, 4555, 0,
...
kernel = next(entry for entry in payload.get("kernels", []) if entry.get("source_kernel") == "mlk")
sites = [
    entry
    for entry in kernel.get("planned_sites", [])
    if entry.get("when") == "memory_op"
    and entry.get("contract") == "memory_op_v1"
    and int(entry.get("event_materialization", {}).get("bytes", {}).get("value", 0) or 0) == 4
]
if len(sites) != 1:
    raise SystemExit(f"expected exactly one 4-byte memory_op site for mlk, found {len(sites)}")
kernel["planned_sites"] = sites
...
echo ""
echo "================================================================================"
echo "Binary Probe Memory-Op dh_comms Tests"
echo "================================================================================"
...
TEST_NAME="binary_probe_memory_op_dh_comms_build"
...
sites = [
    entry
    for entry in kernel.get("planned_sites", [])
    if entry.get("when") == "memory_op" and entry.get("contract") == "memory_op_v1"
]
assert len(sites) == 1
site = sites[0]
assert str(site.get("injection_point", {}).get("instruction_mnemonic", "")) == "flat_store_dword"
assert int(site.get("injection_point", {}).get("instruction_address", 0) or 0) > 39000
...
assert int(target.get("private_segment_fixed_size", 0)) > 0
PY
then
    echo -e "  ${GREEN}✓ PASS${NC} - Memory-op dh_comms regeneration produced a carrier with a planned post-prologue flat-store site"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Memory-op dh_comms carrier regeneration metadata was not correct"
    echo "  Plan: $PLAN_JSON"
...
TESTS_RUN=$((TESTS_RUN + 1))
TEST_NAME="binary_probe_memory_op_dh_comms_runtime"
echo -e "\n${YELLOW}[TEST $TESTS_RUN]${NC} $TEST_NAME"
...
if [ "$run_status" -ne 124 ] && \
   ...
   grep -Eq '"dwarf_line":[[:space:]]*4555' "$RUNTIME_LOG" && \
   ...
   ! grep -q "Memory access fault by GPU" "$RUNTIME_LOG"; then
    echo -e "  ${GREEN}✓ PASS${NC} - Memory-op carrier emitted host-visible dh_comms traffic from a decoded in-range flat-store address"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Memory-op carrier did not produce the expected host-visible dh_comms output"

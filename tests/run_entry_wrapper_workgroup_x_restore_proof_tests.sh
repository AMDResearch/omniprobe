#!/bin/bash
################################################################################
# Entry-wrapper workgroup-x restore proof tests
#
# Validates the next constrained reconstruction slice:
...
if [ ! -f "$MODULE_LOAD_PLAIN_HSACO" ] || [ ! -x "$HSA_LAUNCH_TEST" ]; then
    echo -e "${YELLOW}SKIP: Entry-wrapper workgroup-x restore proof artifacts not built${NC}"
...
WORK_DIR="$OUTPUT_DIR/entry_wrapper_workgroup_x_restore_proof"
rm -rf "$WORK_DIR"
mkdir -p "$WORK_DIR"

PROOF_HSACO="$WORK_DIR/module_load_entry_wrapper_workgroup_x_restore_proof.hsaco"
PROOF_REPORT="$WORK_DIR/module_load_entry_wrapper_workgroup_x_restore_proof.report.json"
PROOF_MANIFEST="$WORK_DIR/module_load_entry_wrapper_workgroup_x_restore_proof.manifest.json"
PROOF_STDOUT="$WORK_DIR/module_load_entry_wrapper_workgroup_x_restore_proof.stdout"
PROOF_STDERR="$WORK_DIR/module_load_entry_wrapper_workgroup_x_restore_proof.stderr"
PROOF_REGEN_DIR="$WORK_DIR/.module_load_entry_wrapper_workgroup_x_restore_proof.hsaco.regen"
PROOF_ASM="$PROOF_REGEN_DIR/output.s"
LAUNCH_OUT="$WORK_DIR/launch.out"

echo ""
echo "================================================================================"
echo "Entry-Wrapper Workgroup-X Restore Proof Tests"
echo "================================================================================"
...
TEST_NAME="entry_wrapper_workgroup_x_restore_report"
echo -e "\n${YELLOW}[TEST $TESTS_RUN]${NC} $TEST_NAME"
echo "  Validate regeneration report declares workgroup_id_x restoration from the hidden handoff"
...
assert entry.get("mode") == "entry-wrapper-workgroup-x-restore-proof"
assert entry.get("wrapper_size") == 52
...
assert len(fields) == 2
assert fields[0]["name"] == "workgroup_id_x"
...
then
    ...
    echo -e "  ${RED}✗ FAIL${NC} - Workgroup-x restore proof report did not match the expected contract"
...
PY
then
    echo -e "  ${GREEN}✓ PASS${NC} - Rebuilt assembly contains the expected workgroup-x restore sequence"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Rebuilt assembly did not contain the expected workgroup-x restore sequence"
    echo "  Assembly saved to: $PROOF_ASM"
    TESTS_FAILED=$((TESTS_FAILED + 1))
fi

TESTS_RUN=$((TESTS_RUN + 1))
TEST_NAME="entry_wrapper_workgroup_x_restore_launch"
echo -e "\n${YELLOW}[TEST $TESTS_RUN]${NC} $TEST_NAME"
echo "  Validate single-workgroup HSA launch restores workgroup_id_x=0 and preserves behavior"
...
if ROCR_VISIBLE_DEVICES="$ROCR_VISIBLE_DEVICES" \
   ...
   --populate-workgroup-id-x 0 \
   --single-workgroup > "$LAUNCH_OUT" 2>&1; then
    echo -e "  ${GREEN}✓ PASS${NC} - Workgroup-x restore proof hsaco launches successfully through the single-workgroup HSA path"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
    echo -e "  ${RED}✗ FAIL${NC} - Workgroup-x restore proof hsaco failed under the HSA path"
